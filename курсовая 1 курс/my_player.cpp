  ...
  for (int y = 0; y < m_rows; y++)
  {
    for (int x = 0; x < m_cols; x++)
    {
      ...
      for (int d = 0; d < 4; d++)
      {
        ...
        if (check_line(x, y, dx, dy, sign) >= 3)
        {
          threat_count++;
...
Point MyPlayer::make_move( const State &state )
{
  ...
  if (find_win_move(result, m_sign))
    return result;
  
  if (find_block_move(result, enemy_sign, 4))
    return result;
  
  if (find_double_threat(result, m_sign))
    return result;
  
  if (find_block_move(result, enemy_sign, 3))
    return result;
  
  if (find_strategic_move(result, m_sign))
    return result;
