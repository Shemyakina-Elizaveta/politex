...
#include "short_array/short_array.h"
#include "worker_db/worker_db.h"
#include "base_file/base_file.h"
#include "bool_array/bool_array.h"
...
int main() {
  ...
  nb->f();
     ...
     */
     ...
     // Нельзя перегрузить первую функцию, т.к. метод всегда имеет неявный первый параметр с типом класса
    ...
    {
        MyString s1("abc");  // 1 выделение памяти
        MyString s2 = s1 + "ccc"; // 2 выделения памяти

        s1 = "abc" + s2; // 3 выделения памяти -> 2
        s2 = std::move(s1); // 1 выделение памяти -> 0

        Matrix m1(4), m2 = m1 + m1; // 1, 1 выделения памяти
        m1 = m2 * m1; // 2 выделения памяти -> 1
        m2 = std::move(m1); // 1 выделение памяти -> 0
    }
...

    {
      ...
      std::cout << "Initial array (size=" << arr.size() << "): ";
...

    {
      ...
      ShortArray arr1(8, 42);
      std::cout << "arr1: size=" << arr1.size() 
                << ", is_heap=" << arr1.is_heap_storage()
                << ", inline_cap=" << arr1.inline_capacity() << "\n";

      ShortArray arr2(20, 99);
      std::cout << "arr2: size=" << arr2.size() 
                << ", is_heap=" << arr2.is_heap_storage() << "\n";

      arr1.push(10);
      arr2.push(30);

      std::cout << "arr1 after push: size=" << arr1.size() 
                << ", is_heap=" << arr1.is_heap_storage() << "\n";
      std::cout << "arr2 after push: size=" << arr2.size() << "\n";

      ShortArray arr3 = arr1;
      std::cout << "arr3 (copy of arr1): size=" << arr3.size()
                << ", is_heap=" << arr3.is_heap_storage() << "\n";

      ShortArray arr4 = std::move(arr2);
      std::cout << "arr4 (moved from arr2): size=" << arr4.size()
                << ", is_heap=" << arr4.is_heap_storage() << "\n";
    ...
    // Неявно определены. Их не должно быть, т.к. если в одном из скопированных BaseFile будет закрыт файл, то второй порушится

    {
      ...
      std::cout << "file1 is_open: " << file1.is_open() << std::endl;
      std::cout << "file2 is_open: " << file2.is_open() << std::endl;

      // BaseFile file3 = file2; // нельзя
