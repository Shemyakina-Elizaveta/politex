...
#include "text_wrapper/text_wrapper.h"
...
int main()
{
    ...
    {
      // Rect rect; // все координаты - мусор
    }
    ...
    {
        // Rect rect1(1, 2, 3, 4); // конструктор
        // Rect rect2 = rect1; // копирующий конструктор, через оператор присваивания
        // Rect rect3(rect1);  // копирующий конструктор
     ...
     */
    {
      // деструктор - спец функция для удаления объекта. вызывается когда объект перестает
      // использоваться (delete или выход из области видимости). 
    }
...

    {
      ...
      Rect r1; // default constructor
      Rect *pR = new Rect(1, 2, 1, 2); // create constructor  
      {
        Rect r2(r1); // cope constructor
        Rect arRect[2]; // 2 default constructor
        for (int i = 0; i < 3; i++)
        {
          static Rect r3(i, i, i, i); // create contructor
          Rect r4(*pR); // copy contructor
          Rect r5(i, i, i, i); // create contructor
        } // destructor r4, r5

        // r4, r5 создаются 3 раза -> удаляются 3 раза
        // r3 создается 1 раз
      } // destructor r2, arRect
      delete pR; // destructor pR
    } // destructor r3
    ...
    // вызывается через объект, имеет доступ к приватным полям 
    ...
    {
      std::cout << "\n\n\n1.10\n\n";
      Rect r1(1, 3, 3, 1), r2(1, 4, 3, -1); // 2 create constructor
      Rect r3 = bounding_rect(r1, r2); // create constructor, copy constructor
      print_rect(r3);
    }
...

    {
      ...
      while ((conc = alch.get_conc()) > 50)
      {
        alch.pout_from(water, 1);
        water.pout_from(alch, 1);
        cnt++;
      }
...

    {
      ...
      Matrix prev(2, 1), cons(2, 2), fib(2, 1);
      ...
      for (int i = 2; i < 41; i++)
      {
        fib = cons.multiply(prev);
        std::cout << fib.get(0, 0) << ' ';
        prev = fib;
      }
...

    {
      ...
      // Here user input is shown. This line must be at most 40 charactersssssssssssss long.
    }
